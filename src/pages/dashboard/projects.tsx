...
type ProjectWithRole = Doc<"projects"> & { _teamRole: "owner" | "editor" | "viewer" };
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-2 mt-4 pt-4 border-t border-border">
          ...
          {project._teamRole !== "owner" && (
            <div className="flex items-center gap-1 text-[10px] font-medium border border-border rounded-full px-2 py-0.5 text-muted-foreground">
              <Users className="w-3 h-3" />
              <span className="capitalize">{project._teamRole}</span>
            </div>
          )}
...
export default function ProjectsPage() {
  ...
  const [filter, setFilter] = useState<"all" | "active" | "archived" | "shared">("all");
  ...
  const typedProjects = projects as ProjectWithRole[];

  const filtered = typedProjects.filter((p) => {
    const matchesSearch = p.name.toLowerCase().includes(search.toLowerCase());
    const matchesFilter =
      filter === "all" ||
      (filter === "shared" && p._teamRole !== "owner") ||
      (filter !== "shared" && p.status === filter);
    return matchesSearch && matchesFilter;
  });

  const sharedCount = typedProjects.filter((p) => p._teamRole !== "owner").length;
  const shareProject = shareProjectId
    ? typedProjects.find((p) => p._id === shareProjectId)
    : null;
  ...
  return (
    <div className="p-6 lg:p-10 max-w-5xl">
      ...
      {typedProjects.length > 0 && (
        ...
        >
          ...
          <div className="flex items-center rounded-xl p-1 border border-border h-10 shadow-sm">
            {(["all", "active", "archived", ...(sharedCount > 0 ? ["shared" as const] : [])] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
      ...
      {/* Project grid */}
      {filtered.length === 0 && typedProjects.length > 0 ? (
        <Empty>
          <EmptyHeader>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((project, i) => (
            <ProjectCard
              ...
              project={project}
              index={i}
              onShare={() => setShareProjectId(project._id)}
            />
          ))}
        </div>
      )}
